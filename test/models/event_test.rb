...
class EventTest < ActiveSupport::TestCase
  test '#created_by? owner_idと 引数の#id が同じ時' do
    event = FactoryBot.create(:event)

    user = Minitest::Mock.new.expect(:id, event.owner_id) # モック
    ...
    # assert_equal(true, event.created_by?(event.owner))  # Userモデルに依存
  end
  test '#created_by? owner_idと 引数の#id が異なる時' do
    event = FactoryBot.create(:event)
